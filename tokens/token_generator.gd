...
func _ready():	
	...
	for point in polygons[0]:
		p.push_back(point - $Sprite.texture.get_size() / 2)
	...
	for i in range(mdt.get_vertex_count()):
		...
		mdt.set_vertex_uv(i, Vector2(vertex.x / $Sprite.texture.get_width() + 0.5, vertex.y  / $Sprite.texture.get_height() + 0.5))
	...
	for i in range(outline.size()):
		if i + 1 >= outline.size():
			break
		st.add_vertex(to_vec3(outline[i]) / scale_factor)
		st.add_vertex(to_vec3(outline[i]) / scale_factor + Vector3(0, 0, width))
		st.add_vertex(to_vec3(outline[i + 1]) / scale_factor)
		st.add_vertex(to_vec3(outline[i]) / scale_factor + Vector3(0, 0, width))
		st.add_vertex(to_vec3(outline[i + 1]) / scale_factor + Vector3(0, 0, width))
		st.add_vertex(to_vec3(outline[i + 1]) / scale_factor)
	st.generate_normals()
	st.commit(mi.mesh)
...
func _draw():
	...
	for i in range(0, points.size(), 3):
		draw_line(points[i] + $Sprite.position, points[i+1] + $Sprite.position, Color.red)
		draw_line(points[i+1] + $Sprite.position, points[i+2] + $Sprite.position, Color.red)
		draw_line(points[i] + $Sprite.position, points[i+2] + $Sprite.position, Color.red)
	for i in p.size() - 1:
		draw_line(p[i] + $Sprite.position, p[i+1] + $Sprite.position, Color.green)
		pass
